...
const Bookcard = ({ datas,favourite }) => {
  ...
  const handleremove = async()=>{
    const response = await axios.put("http://localhost:3000/api/books/favourite/delete-favourite-books",{},{headers})
  ...
  return (
      ...
      <div className='bg-zinc-700 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col items-center p-5'>
        <Link to={`/view-book-detail/${datas._id}`} className="transition-transform duration-300 hover:scale-105">
        ...
        {favourite===true && 
        <button className='text-base flex flex-row font-semibold bg-amber-300/20 px-3 py-2 rounded-2xl shadow-md text-neutral-200 hover:bg-amber-300/80 translate-2.5 transition-all ease-in-out duration-300'
